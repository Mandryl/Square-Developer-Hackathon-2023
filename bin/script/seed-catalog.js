...
const clearCatalog = async () => {
  try {
    const { result: { objects } } = await catalogApi.listCatalog(undefined, "ITEM,ITEM_VARIATION,TAX,IMAGE,CATEGORY,MEASUREMENT_UNIT");
  ...
  // try {
  //   const { result: { objects } } = await catalogApi.listCatalog(undefined, "CUSTOM_ATTRIBUTE_DEFINITION");
  //   if (objects && objects.length > 0) {
  //     objects.forEach(async (object) => {
  //       const { result, errors } = await catalogApi.deleteCatalogObject(
  //         object.id,
  //       );
  //       console.log("Successfully deleted a catalog custom attribute")
  //     });
